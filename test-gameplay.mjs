...
(async () => {
  ...
  // Sample pixels to verify rendering
  const screenCheck = await page.evaluate(() => {
    ...
    const samples = [];
    // Sample center, corners, and some points
    const points = [
      [w/2, h/2],      // center
      [50, 50],        // top-left
      [w-50, 50],      // top-right
  ...
  // Test camera panning with mouse move
  console.log('--- Testing camera pan ---');
  const canvas = await page.$('canvas');
  await canvas.hover();
  
  // Move mouse to right edge to trigger edge pan
  await page.mouse.move(1200, 360);
  await new Promise(r => setTimeout(r, 1000));
  
  // Sample pixels again to see if camera moved
  ...
  await browser.close();
  console.log('--- Done: Game renders and responds to input ---');
})().catch(e => {
